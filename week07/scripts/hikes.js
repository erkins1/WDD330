...
export default class Hikes {
  ...
  showOneHike(hikeName) {
    ...
    document.getElementById("addComment").addEventListener("touchend", addNewComment);
  }
...
function renderOneHikeFull(hike) {
  ...
  let output = ` 
        ...
        <div>
          <h3>Comments</h3>
            ...
            <button id ="addComment" type="button">Submit Comment</button>
          </div>
    `;
  ...
  output += comments.renderCommentList(hike.hikeId);
...
function addNewComment(event) {
  ...
  comment.addComment(event.path[1].id, event.path[1].firstElementChild.value);
  window.location.reload();

  // console.log(event.path[1].firstElementChild.value);
};